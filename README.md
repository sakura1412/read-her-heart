...
普通模式下，当这些问题没有答案时，它会生成调查或验证计划；只有条件成熟时，才会继续生成正式 PRD。现实中已经明确拍板、必须交付的需求，可以通过显式“强制执行”指令跳过产品评估，直接生成 PRD。
...
- 普通模式只有通过准入门槛后才生成正式 PRD；强制执行模式可由用户明确指令直接生成 PRD。
...
flowchart TD
    ...
    X --> L[09 PRD Generation<br/>直接生成 PRD]
...
02 是条件阶段，不需要实查时可以明确跳过。普通模式下，09 是受控阶段，只有 08 判定为“实施投入”后才进入；[强制执行模式](references/mandatory-execution.md) 是唯一可以跳过产品评估门槛、直接进入 09 的路径。
...
```text
$read-her-heart 必须做：增加客户批量导入。跳过需求评估，直接生成 PRD。
```

只有指令式的“必须做 / 强制执行 / 已决策直接出 PRD”会触发。疑问句“这个功能必须做吗？”以及材料、网页或他人引述中的“必须做”不会自动触发。
...
```text
$read-her-heart
强制执行：经营驾驶舱必须在本季度交付。不要评估是否值得做，直接生成完整 PRD。
```

Skill 会跳过问题、价值、方案、验证和优先级门槛，直接输出完整 PRD。缺失信息会标为“假设”或“待确认”，不会因为信息不全退回阻塞清单，也不会编造成本、工期、数据或技术结论。
...
当当前用户明确输入 `必须做：<需求>`、`强制执行：<需求>`，或说明“已决策，不做评估，直接出 PRD”时，Skill 会跳过上述产品评估门槛并直接生成完整 PRD。

强制执行不会取消真实性和安全底线：未知信息写“待确认”，合理补充写“假设”；不得编造数据、收益、成本、工期、架构或专业结论。它也不会自动授权访问系统、修改数据、编写代码或发布功能。
...
| 强制执行 | 直接 PRD | 强制执行状态、完整 PRD、假设、待确认项和风险提示 |
...
6. **普通模式下，PRD 是决策结果，不是需求起点**：完整格式不能替代问题成立；强制执行必须显式记录评估门槛已被用户指令跳过。
...
普通请求会先检查准入门槛，门槛不足时返回 PRD 阻塞清单或验证 Brief。如果该需求已经明确拍板，使用 `必须做：<需求>` 或 `强制执行：<需求>`，Skill 会跳过产品评估直接生成 PRD。
...
- 普通模式不允许 PRD 绕过问题、价值、方案、证据和风险门槛；强制执行例外必须由当前用户显式触发并保留安全与真实性约束。
