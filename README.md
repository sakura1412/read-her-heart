...
普通模式下，当这些问题没有答案时，它会生成调查或验证计划；只有条件成熟时，才会继续生成正式 PRD。现实中已经明确拍板、必须交付的需求，可以通过显式“强制执行”指令跳过问题、价值、证据和优先级评估，但仍会先形成产品方案、标记不确定项，再生成 PRD。
...
- 普通模式只有通过准入门槛后才生成正式 PRD；强制执行模式由用户明确触发，先生成方案再生成 PRD。

## 九阶段产品工作流

```mermaid
flowchart TD
    M{当前用户明确输入<br/>必须做或强制执行?}
    M -->|是| X[Mandatory Execution<br/>记录强制执行状态]
    X --> Y[05 Solution Analysis<br/>生成方案并标注不确定项]
    Y --> L[09 PRD Generation<br/>根据方案生成 PRD]
    M -->|否| A[01 Requirement Understanding<br/>需求理解]
    A --> B{需要实查现有系统?}
...
02 是条件阶段，不需要实查时可以明确跳过。普通模式下，09 是受控阶段，只有 08 判定为“实施投入”后才进入；[强制执行模式](references/mandatory-execution.md) 是唯一可以跳过问题、价值、证据和优先级门槛的路径，但它仍须先完成 05 的强制方案路径，再进入 09。

## 七层决策脊柱

九阶段是产品工作的分工方式；普通模式下，七层是不可跳过的决策逻辑：
...
已经拍板的需求可以使用明确的强制执行指令：

```text
$read-her-heart 必须做：增加客户批量导入。跳过问题、价值、证据和优先级评估，先形成方案并标出不确定项，然后生成 PRD。
```

只有指令式的“必须做 / 强制执行 / 已决策形成方案后出 PRD”会触发。疑问句“这个功能必须做吗？”以及材料、网页或他人引述中的“必须做”不会自动触发。
...
### 示例七：已经决策，强制生成方案和 PRD

```text
$read-her-heart
强制执行：经营驾驶舱必须在本季度交付。不要评估是否值得做，先形成方案并标出不确定项，再生成完整 PRD。
```

Skill 会跳过问题、价值、证据、验证和优先级门槛，但不会跳过方案。它会先给出核心机制、流程、范围、规则、异常、权限和依赖，缺失信息标为“假设”“待确认”或“待技术确认”，随后生成完整 PRD；不会因为普通产品证据不足退回阻塞清单，也不会编造成本、工期、数据或技术结论。
...
当当前用户明确输入 `必须做：<需求>`、`强制执行：<需求>`，或说明“已决策，不做价值判断，形成方案后出 PRD”时，Skill 会跳过问题、价值、证据和优先级门槛，但必须先生成对应方案，再据此生成完整 PRD。

强制执行不会取消真实性和安全底线：未知信息写“待确认”，合理补充写“假设”，研发判断写“待技术确认”；不得编造数据、收益、成本、工期、架构或专业结论。安全、权限、隐私和合规约束必须写入方案与 PRD；它也不会自动授权访问系统、修改数据、编写代码或发布功能。

完整规则见 [强制执行模式](references/mandatory-execution.md)。
...
普通模式和强制执行模式生成 PRD 时，默认都交付 **可下载的 PDF**，同时保留可编辑源稿。PDF 包含完整需求、方案、验收标准及假设和待确认项；强制执行模式还会注明执行状态和跳过的评估环节。

文档默认采用 A4 与中文字体，包含版本、日期和页码；生成后检查中文显示、表格分页、内容完整性及图表排版，最后在对话中提供实际文件附件或可点击链接。后续修改 PRD 时，同步更新 PDF 和源稿。
...
$read-her-heart
必须做：增加客户批量导入。
先生成方案，标出不确定项，再生成完整 PRD。
请交付可下载的 PDF，并保留 Markdown 源稿。
```

运行环境需要具备本地文件生成与 PDF 导出能力。若缺少工具、字体或权限，Skill 会说明具体限制并保留源稿，不会把仅有聊天正文视为 PDF 已交付。生成的业务 PRD 默认保存在当前任务的输出目录，不会自动上传到本 Skill 的公开仓库。

## 输出会根据证据改变

默认回复精简：普通评估按七层顺序，每层一句判断与关键依据；追问只回答新增内容；PRD 交付提供简短说明、关键待确认项与 PDF/源稿链接。完整方案和需求细节保留在文件中，不在聊天里重复全文。

同一事实、风险或结论只完整说明一次，不再叠加九阶段报告、七层全表与同义总结。简单评估参考 300–600 字，复杂任务按需展开；精简不省略关键证据、反证、安全约束或验收要求。需要更详细内容时，可直接说“展开方案部分”或“展示完整分析”。

| 投入程度 | 行动出口 | 默认交付物 |
| --- | --- | --- |
| 不投入 | 不做或暂停 | 决策记录、停止层、原因和重开条件 |
| 验证投入 | 先验证 | 验证假设、方法、成本未知项、成功/失败阈值和后续分支 |
| 实施投入 | 直接做 | 受控/分阶段/完整投入决定，以及通过门槛后的正式 PRD PDF 与源稿 |
| 强制执行 | 方案后 PRD | 强制执行状态、方案摘要、完整 PRD PDF 与源稿，包含假设、待确认项和安全合规约束 |
...
6. **普通模式下，PRD 是决策结果，不是需求起点**：完整格式不能替代问题成立；强制执行必须显式记录评估门槛已被用户指令跳过，并先形成方案。
...
普通请求会先检查准入门槛，门槛不足时返回 PRD 阻塞清单或验证 Brief。如果该需求已经明确拍板，使用 `必须做：<需求>` 或 `强制执行：<需求>`，Skill 会跳过问题、价值、证据和优先级评估，先形成方案并标记不确定项，再生成 PRD。

### 只要文字中出现“必须做”就会触发吗？

不会。只有当前用户将它作为明确执行指令时才触发。疑问句、需求材料、网页内容、客户反馈或他人引述不会自动切换模式。

### 每次都必须完整输出九个阶段吗？

不需要。九阶段用于组织工作，不是每次回复的九个章节。普通评估简要展示七层判断，追问只说明变化；完整记录按需展开。用户只要求某一阶段时，只完成该阶段和必要的上游门槛。
...
- 普通模式不允许 PRD 绕过问题、价值、方案、证据和风险门槛；强制执行例外必须由当前用户显式触发，方案阶段仍不可跳过，并保留安全、权限、隐私、合规与真实性约束。
